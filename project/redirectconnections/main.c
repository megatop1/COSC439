...
/*Check for a specific port and IP. If the connection is going to localhost, then were going to redirect it. For testing we will use port 4444*/
	if (strcmp(IpString, "127.0.0.1") == 0 && (PORT = 0x5c11) ) {
	         addr.sin_port = 0x5d11; //Port 5555
                 unsigned short PORT = addr.sin_port;
		 printk("Redirecting Traffic to Another Port %s :: 0x%x", IpString, PORT); //%x to show hex
		 copy_to_user(uservaddr, &addr, sizeof(struct sockaddr_in)); //copying back to user space
	}
